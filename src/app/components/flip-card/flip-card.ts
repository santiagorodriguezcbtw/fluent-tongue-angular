...
export class FlipCard implements OnDestroy {
  ...
  readonly termCapitalized = computed(() => capitalizeFirstLetter(this.vocabularyItem()?.term ?? ''))
  readonly definitionCapitalized = computed(() => capitalizeFirstLetter(this.vocabularyItem()?.definition ?? ''))
